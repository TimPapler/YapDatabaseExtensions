...
public protocol ReadTransactionType {
    ...
    */
    ...
    - returns: an `Any` if an item existing in the database for this index.
    */
    ...
    - parameter index: a YapDB.Index.
    - returns: an `Any` if a metadata item existing in the database for this index.
    */
...
public protocol WriteTransactionType: ReadTransactionType {
    ...
    - parameter object: the `Any` which will be written.
    - parameter metadata: an optional `Any` which will be written as metadata.
    */
...
extension YapDatabaseReadTransaction: ReadTransactionType {
    ...
    */
    public func readAtIndex(_ index: YapDB.Index) -> Any? {
        return object(forKey: index.key, inCollection: index.collection)
    }
    ...
    - returns: An optional Any.
    */
    public func readMetadataAtIndex(_ index: YapDB.Index) -> Any? {
        return metadata(forKey: index.key, inCollection: index.collection)
    }
}
...
extension YapDatabaseReadWriteTransaction: WriteTransactionType {
    ...
    public func writeAtIndex(_ index: YapDB.Index, object: Any, metadata: Any? = .none) {
        if let metadata: Any = metadata {
...
extension YapDatabaseConnection: ConnectionType {
    public typealias WriteTransaction = YapDatabaseReadWriteTransaction

    public typealias ReadTransaction = YapDatabaseReadTransaction
    ...
    */
    ...
    - parameter completion: a closure which receives T and returns Void.
    */
    public func asyncRead<T>(_ block: @escaping (YapDatabaseReadTransaction) -> T, queue: DispatchQueue = .main, completion: @escaping (T) -> Void) {
        var result: T! = .none
        asyncRead({ result = block($0) }, completionQueue: queue, completionBlock: { completion(result) })
    }
    ...
    - parameter completion: a closure which receives T and returns Void.
    */
    public func asyncWrite<T>(_ block: @escaping (YapDatabaseReadWriteTransaction) -> T, queue: DispatchQueue = .main, completion: ((T) -> Void)?) {
        var result: T! = .none
        asyncReadWrite({result = block($0)}, completionQueue: queue, completionBlock: { completion?(result) })
    }
