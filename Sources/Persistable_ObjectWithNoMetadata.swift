...
extension Readable where
    ...
    func inTransactionAtIndex(_ transaction: Database.Connection.ReadTransaction) -> (YapDB.Index) -> ItemType? {
        return { index -> ItemType? in
            return self.inTransaction(transaction, atIndex: index)
        }
    }
    ...
    func atIndexesInTransaction<
        Indexes>(indexes: Indexes) -> (Database.Connection.ReadTransaction) -> [ItemType] where
        Indexes: Sequence,
    ...
    */
    public func atIndexes<Indexes>(indexes: Indexes) -> [ItemType] where
        Indexes: Sequence,
        Indexes.Iterator.Element == YapDB.Index {
